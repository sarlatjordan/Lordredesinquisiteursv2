...
interface OperationDetailProps {
  ...
  ships: Pick<
    Ship,
    "id" | "name" | "model" | "ship_type" | "is_org_ship" | "status"
  >[];
}
...
export function OperationDetail({
  ...
  const confirmedRegistrations = (op.registrations ?? []).filter(
    ...
    <div className="space-y-6">
      ...
      >
                ...
                <Dialog>
                  ...
                  <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Cette action supprime définitivement &laquo; {op.title}{" "}
                        &raquo; et toutes les inscriptions associées.
                      </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
        ...
        <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
          ...
          {durationLabel && (
            <span className="flex items-center gap-1.5">
              <Timer className="h-4 w-4 text-primary" />~{durationLabel}
            </span>
          )}
          <span className="flex items-center gap-1.5">
            <Users className="h-4 w-4 text-primary" />
            {op.registration_count ?? 0} inscrit
            {(op.registration_count ?? 0) !== 1 ? "s" : ""}
          </span>
        </div>
          ...
          <div className="flex items-center gap-2">
            <Avatar className="h-7 w-7">
              ...
              <AvatarFallback className="text-[10px] bg-primary/10 text-primary">
                {(op.commander.display_name ?? op.commander.username ?? "C")
                  .slice(0, 2)
                  .toUpperCase()}
              </AvatarFallback>
            </Avatar>
            <span className="text-sm text-muted-foreground">
              ...
              <span className="text-foreground font-medium">
                {op.commander.display_name ?? op.commander.username}
              </span>
            </span>
          </div>
        )}
            ...
            >
              ...
              <div className="rounded-lg border border-border bg-muted/20 p-4">
                <div className="text-sm text-foreground leading-relaxed">
                  <MarkdownContent>{op.description}</MarkdownContent>
                </div>
              </div>
            </motion.section>
          ) : null}
              ...
              <div className="space-y-1.5">
                {(op.role_slots ?? []).length === 0 ? (
                  ...
                  (op.role_slots ?? []).map((slot) => {
                    ...
                    const assignedName =
                      slot.assigned_profile?.display_name ??
                      slot.assigned_profile?.username;
                    const shipName = slot.ship_id
                      ? (ships.find((s) => s.id === slot.ship_id)?.name ?? null)
                      : null;
                    return (
                      <div
                        key={slot.id}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 text-right">
                          <span
                            className={`text-xs ${assignedName ? "text-green-400" : "text-muted-foreground"}`}
                          >
                            {assignedName ?? "—"}
                          </span>
                          {shipName && (
          ...
          {(isCommanderOrManager || (op.resources ?? []).length > 0) && (
            ...
            >
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">
                Ressources (
                {
                  (op.resources ?? []).filter((r) => r.status !== "released")
                    .length
                }
                )
              </h3>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Bilan post-opération — Inquisiteur+ ou commandant
              </p>
            </div>
          </div>
          ...
          {canDebrief ? (
            <div className="rounded-xl border border-border bg-card p-4 space-y-3">
              <Label htmlFor="debrief" className="sr-only">
                Débrief
              </Label>
              <MarkdownEditor
                id="debrief"
